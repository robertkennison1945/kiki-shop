...
if (!VmConfig::get('use_as_catalog', 0)  ) { ?>
	...
	if (($stockhandle == 'disableit' or $stockhandle == 'disableadd') and ($product->product_in_stock - $product->product_ordered) < 1) { ?>
		<span class="vm-details-button"><a href="<?php echo JRoute::_ ('index.php?option=com_virtuemart&view=productdetails&layout=notify&virtuemart_product_id=' . $product->virtuemart_product_id); ?>" class="notify jutooltip product-details" title ="<?php echo vmText::_ ('COM_VIRTUEMART_CART_NOTIFY') ?>"><i class="fa fa-bell"></i><?php echo vmText::_ ('COM_VIRTUEMART_CART_NOTIFY') ?></a></span><?php
	} else {
		$tmpPrice = (float) $product->prices['costPrice'];
		if (!( VmConfig::get('askprice', true) and empty($tmpPrice) ) ) { 
		
			if(JRequest::getVar('view') != 'productdetails' && !$product->orderable) { ?>
				<span class="vm-details-button"><a class="viewproductdetails product-details jutooltip" title ="<?php echo vmText::_ ('COM_VIRTUEMART_ADDTOCART_CHOOSE_VARIANT') ?>" href="<?php echo JRoute::_ ('index.php?option=com_virtuemart&view=productdetails&virtuemart_product_id=' . $product->virtuemart_product_id); ?>"><i class="fa fa-cart-plus"></i><?php echo vmText::_ ('COM_VIRTUEMART_ADDTOCART_CHOOSE_VARIANT') ?></a></span>
			<?php } else { ?>
